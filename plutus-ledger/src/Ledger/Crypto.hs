...
sign msg privKey = Signature . toBuiltin . Crypto.unXSignature $ Crypto.sign passPhrase (toXPrv privKey) msg

passPhrase :: BS.ByteString
passPhrase = "Ledger.Crypto PassPhrase"

generateFromSeed :: BS.ByteString -> PrivateKey
generateFromSeed seed = fromXPrv $ Crypto.generate seed passPhrase

toXPrv :: PrivateKey -> Crypto.XPrv
toXPrv = either error id . Crypto.xprv . KB.bytes . getPrivateKey

fromXPrv :: Crypto.XPrv -> PrivateKey
fromXPrv = PrivateKey . KB.fromBytes . Crypto.unXPrv

privateKeyToXPub :: PrivateKey -> Crypto.XPub
privateKeyToXPub = Crypto.toXPub . toXPrv

xPubToPublicKey :: Crypto.XPub -> PubKey
xPubToPublicKey = PubKey . KB.fromBytes . Crypto.xpubPublicKey

toPublicKey :: PrivateKey -> PubKey
toPublicKey = xPubToPublicKey . privateKeyToXPub
