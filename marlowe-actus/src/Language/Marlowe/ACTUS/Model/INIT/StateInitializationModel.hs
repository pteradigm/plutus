...
_INIT ct@ContractTerms{..} = let
    ...
    in do
      ...
      tmd <- case contractType of
                    PAM -> ct_MD
                    _   -> let tminus' | isJust ct_PRANX && ct_PRANX >= Just t0     = ct_PRANX
      ...
      nt <- let nt | ct_IED > Just t0 = Just 0.0
                   | otherwise   = (\x -> r ct_CNTRL * x) <$> ct_NT
            in nt

      ipnr <- let ipnr | ct_IED > Just t0 = Just 0.0
                       | otherwise   = ct_IPNR
              in ipnr

      ipac <- let ipac | isNothing ct_IPNR = Just 0.0
                       | isJust ct_IPAC    = ct_IPAC
                       | otherwise         = (\d -> y d tminus t0 ct_MD * nt * ipnr) <$> ct_DCC
              in ipac

      feac <- let feac | isNothing ct_FER     = Just 0.0
      ...
      nsc   <- let nsc | maybe False scef_xNx ct_SCEF = ct_SCCDD
                       | otherwise                    = Just 1.0
               in nsc

      isc   <- let isc | maybe False scef_Ixx ct_SCEF = ct_SCCDD
                       | otherwise                    = Just 1.0
               in isc
      ...
      prnxt <- case contractType of
            PAM -> Just 0.0
            LAM -> let s | isJust ct_PRANX && ct_PRANX > Just t0                           = ct_PRANX
            ...
            ANN ->
              let prnxt | isJust ct_PRNXT = liftA2 (*) (Just $ r ct_CNTRL) ct_PRNXT
                        | otherwise       = Just $ ipac * nt
              in prnxt

      ipcb  <- case contractType of
                PAM -> Just 0.0
                _   -> let ipcb | Just t0 < ct_IED        = Just 0.0
