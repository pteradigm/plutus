...
payoff ev RiskFactors{..} ContractTerms{..} ContractStatePoly{..} t =
    let y_sd_t = _y (fromJust ct_DCC) sd t ct_MD
    in case contractType of

        PAM ->
            case ev of
                IED -> _POF_IED_PAM o_rf_CURS ct_CNTRL (fromJust ct_NT) (fromJust ct_PDIED)
                MD  -> _POF_MD_PAM o_rf_CURS nsc nt isc ipac (if isNaN feac then 0.0 else feac)
        ...
        ANN ->
            case ev of
                ...
                PY  -> _POF_PY_PAM (fromJust ct_PYTP) o_rf_CURS o_rf_RRMO (fromJust ct_PYRT) ct_cPYRT ct_CNTRL nt ipnr y_sd_t
                FP  -> _POF_FP_PAM (fromJust ct_FEB) (fromJust ct_FER) o_rf_CURS ct_CNTRL nt feac y_sd_t
                PRD -> _POF_PRD_LAM o_rf_CURS ct_CNTRL (fromJust ct_PPRD) ipac ipnr nt y_sd_t
                TD  -> _POF_TD_LAM o_rf_CURS ct_CNTRL (fromJust ct_PTD) ipac ipnr nt y_sd_t
                IP  -> _POF_IP_LAM o_rf_CURS isc ipac ipnr nt y_sd_t
                _   -> 0.0
